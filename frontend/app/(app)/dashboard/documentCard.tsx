...
export default function DocumentCard(props: documentProps) {
    ...
    const dropdown: DropdownInterface = new Dropdown(
        document.getElementById(props.id + "menu"), 
        document.getElementById(props.id + "trigger")
        )
    ...
    return(
        <div className="w-full h-20 bg-gray-700 rounded-md py-2 px-4">   
            <div className="mb-2">
                ...
                <button className="float-right -mt-2 text-2xl select-none" id={(props.id as string) + "trigger"}>...</button>
                <div id={(props.id as string) + "menu"} className="z-10 hidden divide-y divide-gray-100 rounded-lg shadow w-44 bg-gray-800">
                    <ul className="py-2 text-sm  text-gray-200" aria-labelledby="dropdownHoverButton">
                    <li>
                        <a onClick={deleteEnv} className="block px-4 py-2  hover:bg-gray-600 hover:text-white">Delete</a>
                    </li>
                    </ul>
                </div>
            </div>
